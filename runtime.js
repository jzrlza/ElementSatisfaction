...
function w(n, i, o, a) {
	let s, l, u, c, d = t(n);
	n instanceof Map ? n instanceof n.constructor ? (s = `Map(${n.size})`, l = y) : (s = "Map()", l = S) : n instanceof Set ? n instanceof n.constructor ? (s = `Set(${n.size})`, l = g) : (s = "Set()", l = S) : d ? (s = `${n.constructor.name}(${n.length})`, l = x) : (c = m(n)) ? (s = `Immutable.${c.name}${"Record"===c.name?"":` ($ {
			n.size
		})
		`}`, d = c.arrayish, l = c.arrayish ? C : c.setish ? E : P) : a ? (s = f(n), l = N) : (s = f(n), l = S);
...
function k(e, n, i, o) {
	let a, s, l, u, c = t(e);
	if(e instanceof Map ? e instanceof e.constructor ? (a = `Map(${e.size})`, s = O) : (a = "Map()", s = D) : e instanceof Set ? e instanceof e.constructor ? (a = `Set(${e.size})`, s = T) : (a = "Set()", s = D) : c ? (a = `${e.constructor.name}(${e.length})`, s = R) : (u = m(e)) ? (a = `Immutable.${u.name}${"Record"===u.name?"":` ($ {
				e.size
			})
			`}`, c = u.arrayish, s = u.arrayish ? U : u.setish ? A : F) : (a = f(e), s = D), n) {
		const t = document.createElement("span");
...
var ge = Ee((async function(e, t) {
	...
	const r = await ye(n);
	return `${he}${r.name}@${r.version}/${n.path||function(e){for(const t of _e){const n=e[t];if("string"==typeof n)return be.test(n)?n:`
	$ {
		n
	}.js `}}(r)||"index.js"}`
}));
